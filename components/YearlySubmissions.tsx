...
const YearlySubmissions = ({activeDays, maxStreak, currentStreak}: {activeDays: number | undefined, maxStreak: number | undefined, currentStreak: number | undefined}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (user && user?.id) {
          const response = await fetch("/api/activity/yearly/" + user.id);
  ...
  return (
    <div className="relative">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="text-sm text-gray-400">Total active days: {activeDays ? activeDays : ''} &nbsp; Max streak: {maxStreak !== undefined && currentStreak !== undefined ? (currentStreak > maxStreak ? currentStreak : maxStreak) : ''}</div>
      </div>
...
export default YearlySubmissions;
